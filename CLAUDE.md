...
**QUEUED DESIGN — NOT YET EXECUTED (recorded 05-29-26):** Two major functional changes were fully designed in discussion and approved by Doug, but **no document edits have been made** — the design lives only as the decisions recorded here, to be executed later as a single batched Gate 2 carry-forward (held deliberately to avoid version churn across the overlapping documents). **Nothing described in this entry exists in the documents yet:** the Event entity has no `listedPublicly`/`publishedAt` fields, `recordingUrl` has not been re-defined, there is no Resource entity, there is no CR-RESOURCES sub-domain, and the CR Domain PRD and Domain Overview still say "four sub-domains." A future session must execute this before treating any of it as real.

*Change 1 — Event publishing and public discovery.* The Event lifecycle gains the public-discovery surface it currently lacks (today the model assumes a registrant already holds a specific event's `registrationUrl`; there is no browsable list of available events). Decisions: (a) publishing is a separate, reversible flag on the Event — `listedPublicly` (boolean) plus `publishedAt` (datetime, system-set on first listing) — distinct from the Scheduled status; an explicit "publish to the list" administrator step; auto-unlist on Cancel or Postpone; listing decoupled from whether registration is allowed. (b) The public events list and self-service registration are hosted in the **custom web application** (not WordPress), reusing the existing Contact match-or-create and the website-domain Account precedence ladder. (c) The list shows listed, Scheduled, not-yet-started events ordered soonest-first and drops an event off at its start; existing registration rules (open while Scheduled, blocked after the end date) are unchanged.

*Change 2 — Resources page (recordings + assets).* A new fully-public Resources page lists recordings of completed events and standalone, purpose-built assets. Decisions: (a) a single unified **Resource** entity (new — does not exist yet) with an optional Event link — recordings link to their source Event, standalone assets have no Event. (b) The page is fully public; the publish flag is the only gate, no authentication, so the Resource entity needs no access attributes. (c) `recordingUrl` on the Event is **retained** (reversing an in-discussion deprecation idea) as the raw-source reference — a link into the recording system (typically the virtual meeting platform) the administrator uses to retrieve the unedited recording, then download/edit/re-host. It is auto-populated by the virtual meeting platform integration when a recording is available, with **manual entry as the universal fallback**; editable (auto-population fills only when empty, manual values preserved); and visible for **all** formats (expanded from Virtual/Hybrid so in-person recordings can be linked) — the `format` field description drops `recordingUrl` from its driven-visibility list, and the old "recording and publishing out of scope" note is replaced. The edited, published copy is a **separate Resource** with its own URL (a different artifact than the raw recording); the migration is a documented **manual** workflow off the tail of CR-EVENTS-MANAGE. (d) The Resource entity is owned by a **new CR-RESOURCES sub-domain** — the Client Recruiting domain's **fifth**. Still draftable (no decision required, to be proposed at execution): the Resource field set, a resource type/category scheme, and the CR-RESOURCES process structure.

*Batched execution scope (when run):* Event entity (`listedPublicly`, `publishedAt`, public-visibility/list logic, `recordingUrl` re-definition, `format` description tweak, new data-item IDs), CR-EVENTS-MANAGE (publish/discovery steps and the recording-migration step), a new Resource Entity PRD, a new CR-RESOURCES Sub-Domain Overview and process document, the Events Sub-Domain Overview, the CR Domain PRD and CR Domain Overview (register the fifth sub-domain, "four" → "five"), and an implementation note in the `dbower44022/crmbuilder` custom-application documentation. Version-bump and change-log every touched document; present a Change Summary at the end. ARCHITECTURE mode; run as one carry-forward.

**Latest structural change (05-29-26):** CR-EVENTS virtual meeting platform integration reconciliation. The automated virtual-meeting-platform ("Zoom") integration had been added to the Event and Event Registration Entity PRDs on 05-05-26 (commits `3c27a9b`, `517ee6d`) but never propagated to the upstream documents, which still described virtual attendance as manual-only and carried CR-EVENTS-MANAGE-ISS-001 as a blanket deferral. Doug chose Option A: ratify the conditional integration and propagate it upstream, abstracting the product name and fixing identifier collisions. Five documents updated and committed: **Event Entity PRD v1.1 → v1.2**, **Event Registration Entity PRD v1.1 → v1.2**, **CR-EVENTS-MANAGE v1.0 → v1.1**, **CBM-SubDomain-Overview-Events (CR-EVENTS SDO) v1.0 → v1.1**, and **CR Domain PRD v1.3 → v1.4**, all stamped 05-29-26 14:00. **Product-name abstraction (per the no-product-names standard):** every "Zoom" mention in the two Entity PRDs replaced with "the virtual meeting platform"; the external reference doc is now cited as "Virtual Meeting Platform Integration Architecture" rather than by its product-named repo path. **Field renames:** `zoomWebinarId`→`virtualMeetingWebinarId`, `zoomHostEmail`→`virtualMeetingHostEmail`, `zoomLastSyncedAt`→`virtualMeetingLastSyncedAt`, `zoomConfig`→`virtualMeetingConfig`, `zoomRegistrantId`→`virtualMeetingRegistrantId`; `attendanceCapturedAt` keeps its (already neutral) name. **Identifier-collision fix:** the five integration fields had been assigned `CR-EVENTS-MANAGE-DAT-034`–`037` (Event) and `-045` (Event Registration), which collided with existing Event Registration field IDs in the process document (event/contact links, registrationDate, registrationSource, contactType-append). They were reassigned to the next free block, **CR-EVENTS-MANAGE-DAT-049 through DAT-053**, and added to the process document's Section 8 (Data Collected) as a new "Virtual Meeting Platform Integration Data Items" subsection. **Semantic reconciliation:** the attendance workflow step (step 17) in both the process document and the Domain PRD now describes automated capture when a deployment enables the integration, with manual transcription as the fallback for non-integrated deployments and other platforms; the attendance-mechanism decision cell and the SDO Section 4.7 Attendance Capture Model were updated to match; CR-EVENTS-MANAGE-ISS-001 was rescoped from a blanket deferral to "platform-agnostic import for non-integrated platforms only" in all three documents that carry it (process doc, Event Entity PRD, CR Domain PRD). **Mechanical:** Change Log sections were added to the two Entity PRDs, the process document, and the SDO (none had one); a v1.4 row was appended to the Domain PRD's existing Section 8 Change Log. All four newly-built documents validate clean. The CR Domain PRD shows two pre-existing schema quirks (393 table cells with `shd` before `tcBorders`, and the `<w:zoom>` display-zoom setting missing its `percent` attribute) that are present identically in the git original (verified via `git show HEAD:`) and were left untouched per the minimal-changes principle. This entry supersedes the 05-05-26 "Zoom integration Entity PRD updates" note below: those fields are now product-neutral, upstream-consistent, and collision-free. The conditional design (manual baseline + optional bridge, gated on `virtualMeetingConfig`) is unchanged — EVT-DEC-011/012 still govern.

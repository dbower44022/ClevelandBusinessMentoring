...
**Latest structural change (05-18-26 — Account Entity PRD v1.11 pre-Phase-8 corrections):** Three follow-on corrections to v1.10 from the pre-Phase-8 review pass, executed as a single F1+F3+F4 bundle. **F1**: corrected typo `nature_of_assistance_sought` → `natureOfAssistanceSought` in `PRDs/entities/business-profile-field-inventory.yaml` to align with the YAML schema v1.2.4 lowerCamelCase rule. The YAML and PRD already used the correct name; only the source-of-record inventory had the typo. Inventory bumped v0.1.0 → v0.2.0 (MINOR because F4 also lands in this bump). **F3**: appended a clarifying note to the descriptions of the three percent-owned fields (`percentOwnedByWomen`, `percentOwnedByMinority`, `percentOwnedByVeterans`) in PRD Section 3.6.2 and the corresponding YAML field descriptions, stating that the percentages are independent measurements and may sum to more than 100 percent for owners belonging to multiple groups (for example, a veteran-owned women-owned business). Matches federal certification rules where each is a separate eligibility threshold; the note prevents misreading the three fields as components of a single 100-percent total. **F4**: added new field `businessProfileLastReviewed` (date) to PRD Section 3.6.15 Mentoring Context and the YAML, to provide the freshness signal needed to keep state fields current. Many fields in the profile (`currentlyHiring`, `currentlySeekingCapital`, `bookkeepingStatus`, `revenueTrend`, and similar state attributes) drift month over month; a saved-view filter on this date will surface Client records older than N months for periodic-review nudges to assigned mentors. YAML `content_version` bumped 1.0.0 → 1.1.0 (MINOR per the schema rule for new fields). Total YAML field count rises 81 → 82. **PRD metadata**: Version 1.10 → 1.11, Last Updated 05-18-26 06:00 → 05-18-26 07:00, Status retained as Draft. New ACT-DEC-017 added to Section 9 documenting the v1.11 change in full. **Deferred discussion items** from the same review pass remain on the backlog for future revisions: F2 (`legalEntityType` vs `organizationType` overlap — the existing 2-value enum For-Profit/Non-Profit and the new 12-value enum can drift; three options on the table — keep both, deprecate `organizationType` via derivation formula, or add a validation note), F5 (per-cert metadata limitation on flat-multiEnum `certificationsHeld` and `certificationsInterestedIn` — known structural deferral, reminder for mentors who plan to actively help clients pursue certifications), F6 (UX preview against test EspoCRM before Phase 9 deployment to validate the 9-tab layout grouping), F7 (brand-name precedent confirmation for `hasGoogleBusinessProfile` and the social-media platform multiEnum — established exception per existing `linkedInPage` field, but worth confirming). The v1.11 docx grew 937 → 943 paragraphs (+6). All docx validations passed. **Next:** Phase 8 Stakeholder Review can now proceed against v1.11. The F1+F3+F4 changes are purely additive and clarifying; no pre-existing content was modified other than the three percent-owned field descriptions (note appended) and the metadata header. This entry supersedes the prior 05-18-26 entry's "Next" pointer to Phase 8 — Phase 8 is now the immediate next step rather than v1.10's Phase 8.

**Prior structural change (05-18-26 — Account Entity PRD v1.10 Client Business Profile):** Account Entity PRD v1.9 → v1.10 adding a new Section 3.6 "Client Business Profile" with 81 custom fields organized across 15 functional categories (Identity and Legal, Ownership and Control, Certifications, Industry Classification, Financials, Workforce, Customers and Markets, Sales and Marketing, Operations, Financial Infrastructure, Funding and Capital, Technology, Professional Advisors, Insurance and Compliance, Mentoring Context). All 81 fields scoped to visibleWhen accountType contains Client and MN-domain owned. Implementation companion: new YAML file `programs/MN/MN-Account-business-profile.yaml` v1.0.0, sibling to the existing `MN-Account.yaml` (which keeps its five foundational fields unchanged). Driver: Doug's stated goal of comprehensive business-profile capture so any assigned mentor can review a Client record without re-asking foundational questions session over session. Research basis: SBA Form 641 (federal counseling intake standard used by SCORE, SBDC, and Women's Business Centers), SBA federal certification programs (8(a), WOSB/EDWOSB, VOSB/SDVOSB, HUBZone, SDB, DBE), Ohio Department of Administrative Services certifications (MBE, WBE, EDGE, VFBE), Cuyahoga County certifications, and standard small-business mentoring intake practices — captured in `business-profile-field-inventory.yaml` v0.1.0 (88 candidate fields across 15 categories). Gate 1 scope approval from Doug 05-18-26: 81 of 88 candidate fields included; 7 deferred (`mostRecentFullYearRevenue` — exact dollar sensitive, bracketed `annualRevenueRange` covers the mentoring need; `personalCreditScoreRange` — wrong entity, will be relocated to Contact in a future narrow PRD update; `totalCapitalRaisedToDate`, `fundingSourcesUsedToDate`, `capitalSoughtAmount` — structural decision pending around modeling funding events as a child entity; `otherCoachOrMentorRelationships` and `advisorNotes` — structural decision pending around modeling professional advisors as a child entity). Three categories (Certifications, Funding, Professional Advisors) carry a structural-decision flag: v1.10 commits to flat fields as the first-pass choice; restructure to child entities is flagged as a candidate for a future major revision if per-record tracking of issue dates, expiration dates, amounts, or contact details becomes important. Section 3.6 placement deviation from the original kickoff: the kickoff prompt asked for placement at "Section 3.3.x", but the current PRD's Section 3.3 is Partner-Specific Fields, not Client (Client-specific fields live in Section 3.2). To stay additive without restructuring existing 3.2, the new content was placed as a peer section after the existing 3.5 Incomplete Domain Fields and before Section 4 Relationships. Two new entries added to the PRD's tracking tables: ACT-ISS-005 (Section 8 Open Issues) documenting the 7-field deferral list with paths back for each, and ACT-DEC-016 (Section 9 Decisions Made) documenting the v1.10 change and its full rationale. PRD metadata bumped Version 1.9 → 1.10 and Last Updated 05-05-26 16:04 → 05-18-26 06:00; Status retained as Draft per v1.7-v1.9 convention. The v1.10 docx grew from 498 to 937 paragraphs (+439). Production methodology: Gate 2 execution followed Gate 1 scope approval per the two-gate carry-forward pattern. The companion YAML was filtered from the entity-shape draft `MN-Account-business-profile-draft.yaml` v0.1.0 produced earlier in the same session by removing the 7 deferred fields and adjusting the description block, content_version, and source comment to reference Account Entity PRD v1.10 Section 3.6. The new YAML uses all standard schema v1.2.4 patterns: visibility predicates (single-clause `visibleWhen` for type-conditional fields, `all:` structured form for compound conditions on conditional fields like `exportCountries`, `capitalSoughtPurpose`), 14 enums and 8 multiEnums with full options lists, 31 bool fields, 15 text fields, 10 ints with min/max, 3 currency, 5 varchars (with maxLength on those that need length cap), 1 date, 1 wysiwyg; layout panels covering 9 detail-tab groupings. Open Issue ACT-ISS-005 captures the deferral list as PRD-tracked technical-debt items rather than session-only notes; the personalCreditScoreRange relocation to Contact specifically is noted as needing a separate narrow Contact Entity PRD v1.7 → v1.8 update in a future session. **Next:** Phase 8 Stakeholder Review for Account Entity PRD v1.10 (the v1.9 → v1.10 change is purely additive — new Section 3.6, new ACT-ISS-005, new ACT-DEC-016 — and does not modify any pre-existing content); Phase 9 deployment of `MN-Account-business-profile.yaml` against the test EspoCRM instance can proceed independently once a stakeholder review window is scheduled.

**Prior structural change (05-30-26): CR-RESOURCES draftable items confirmed; all flagged open issues resolved.** Doug reviewed the new CR-RESOURCES documents and confirmed every item that the prior entry had flagged as "not yet confirmed by Doug." Decisions: (1) the `resourceType` value list (Recorded Event; Document; Video; Audio; Template; Link; Other) and the `category` value list (Starting a Business; Financing; Marketing and Sales; Operations; Leadership and Strategy; Legal and Compliance; Other) are **confirmed as-is**; (2) `url`/`file` presence rule — **at least one required before listing, both permitted simultaneously**; (3) public Resources page ordering — **by `category`, then `resourceDate` descending**; (4) unlisted Resources are **retained and never hard-deleted** (unlisting sets `listedPublicly=false` only). On the two ambiguous CR Domain PRD count references, Doug ruled **CR-RESOURCES does NOT participate in gap targeting and carries no subject persona** — it is audience-agnostic public content — so both "all four sub-domains" references **stay "four"** and the CR Domain PRD needs no edit. These resolutions were applied to three documents, each bumped v1.0 → v1.1 via its committed generator: **Resource Entity PRD v1.1** (RES-ISS-001–004 → new decisions RES-DEC-007–010; Open Issues section now empty), **CR-RESOURCES-MANAGE v1.1** (ISS-001–003 resolved as Interview Transcript Q/Decision entries), and **CR-RESOURCES Sub-Domain Overview v1.1** (ISS-001–002 resolved, pointing to the authoritative decisions in the other two). All three regenerated docx were verified (v1.1, Open Issues "None", no residual "pending"/"first-draft" text). Generators were regenerated with `docx` installed in a throwaway `/tmp/cbmbuild` dir (`NODE_PATH=/tmp/cbmbuild/node_modules node …`) since `docx` is not installed in this repo. Status on all three remains **Draft — awaiting Phase 8 Stakeholder Review**. **Next:** Phase 8 stakeholder review, then Phase 9 YAML generation for the new sub-domain (`CR-Resource.yaml` placeholder in the CR Domain PRD Entity Relationships table). This entry resolves the "Open / draftable items flagged for review" and "Next" notes in the prior entry below.

**Prior structural change (05-29-26 — evening): CR-EVENTS event publishing and public discovery, plus the new CR-RESOURCES sub-domain.** The two functional changes recorded as queued earlier today were executed as a single batched carry-forward and pushed. **Change 1 (event publishing and discovery):** Event Entity PRD v1.2 → v1.3 (commit `7d80abd`) added Section 3.6 Publishing Fields — `listedPublicly` (CR-EVENTS-MANAGE-DAT-054) and `publishedAt` (DAT-055) — and re-defined `recordingUrl` (DAT-030) as the raw-source reference (auto-populated by the virtual meeting platform integration when a recording is available, manual entry fallback, editable, visible for all formats), dropped `recordingUrl` from the `format` driven-visibility list, and added EVT-DEC-013/014. CR-EVENTS-MANAGE v1.1 → v1.2 (commit `fd13902`) added the publish-to-list and public browse-and-register workflow steps (now steps 6–7), the optional recording-migration step (now step 21), automatic unlisting on Cancel/Postpone, data items DAT-054/055, and requirements REQ-055–057. **Change 2 (Resources page) — new CR-RESOURCES sub-domain, the FIFTH in Client Recruiting (CR now has CR-PARTNER, CR-MARKETING, CR-EVENTS, CR-REACTIVATE, CR-RESOURCES):** new Resource Entity PRD v1.0 (commit `4b01073`), new CR-RESOURCES-MANAGE process document v1.0 (commit `eaae660`), and new CR-RESOURCES Sub-Domain Overview v1.0 (commit `e5f6a3b`), each with a committed generator (`generate-Resource-Entity-PRD.js`, `generate-CR-RESOURCES-MANAGE.js`, `generate-CBM-SubDomain-Overview-Resources.js` — these new generators ARE current and runnable, unlike the Event/EventRegistration generators which remain stale per the Zoom note below). The Resource entity is a single unified entity for event recordings and standalone assets, fully public (gated solely by `listedPublicly`, no access attributes), with an optional `sourceEvent` link to Event (CR-RESOURCES-MANAGE-DAT-001–010); its published `url`/`file` is the edited copy, distinct from the Event's raw `recordingUrl`. The recording-migration step is a documented manual workflow off the tail of CR-EVENTS-MANAGE. **Domain-level reflection:** Events Sub-Domain Overview v1.1 → v1.2 (commit `8c4eb5d`; new Section 4.11 public listing/discovery + a Section 5 CR-RESOURCES handoff); CR Domain Overview v1.4 → v1.5 (commit `fe7a603`) and CR Domain PRD v1.4 → v1.5 (commit `2b6972b`) register CR-RESOURCES as the fifth sub-domain — the structural count statements changed four → five, and the sub-domain table, process inventory, a new sub-domain subsection, the Resource entity, and the Resource → Event relationship were added. **Deliberately NOT changed in the CR Domain PRD:** the Contact-relationship references that span the four contact-using sub-domains were left as “four” (the Resource entity links to Event, not Contact), and the “all four domains (MN/MR/CR/FU)” reference is unchanged. The CR Domain PRD validation profile is unchanged from the prior version (the pre-existing shd-before-tcBorders quirk count is 393 in both the new and prior versions; the `<w:zoom>` percent quirk is also pre-existing). **crmbuilder:** a custom-application implementation note for the public events list and Resources page was committed to `dbower44022/crmbuilder` at `PRDs/product/CustomApp/custom-app-public-surfaces-events-resources.md` (commit `b1cd61a`); the crmbuilder CLAUDE.md was not updated, per its scope rule. **Open / draftable items flagged for review (not yet confirmed by Doug):** (1) the Resource `resourceType` and `category` value lists, the Resources-page default sort/grouping (proposed: by category, then `resourceDate` descending), and the `url`/`file` presence rule are first-draft proposals (RES-ISS-001–004, CR-RESOURCES-MANAGE-ISS-001–003); (2) two genuinely ambiguous count references in the CR Domain PRD — “gap targeting orients all four sub-domains” and “subject persona across all four sub-domains” — were left as “four” pending Doug’s decision on whether CR-RESOURCES participates in gap targeting and the subject-persona span. **Next:** Doug’s review of the new CR-RESOURCES documents and the flagged draftable items; then Phase 8 stakeholder review and Phase 9 YAML generation for the new sub-domain (a `CR-Resource.yaml` is referenced as a Phase-9 placeholder in the CR Domain PRD Entity Relationships table). This entry supersedes the QUEUED DESIGN note recorded earlier today, which is now fully executed.

**Prior structural change (05-29-26 — virtual meeting platform reconciliation):** CR-EVENTS virtual meeting platform integration reconciliation. The automated virtual-meeting-platform ("Zoom") integration had been added to the Event and Event Registration Entity PRDs on 05-05-26 (commits `3c27a9b`, `517ee6d`) but never propagated to the upstream documents, which still described virtual attendance as manual-only and carried CR-EVENTS-MANAGE-ISS-001 as a blanket deferral. Doug chose Option A: ratify the conditional integration and propagate it upstream, abstracting the product name and fixing identifier collisions. Five documents updated and committed: **Event Entity PRD v1.1 → v1.2**, **Event Registration Entity PRD v1.1 → v1.2**, **CR-EVENTS-MANAGE v1.0 → v1.1**, **CBM-SubDomain-Overview-Events (CR-EVENTS SDO) v1.0 → v1.1**, and **CR Domain PRD v1.3 → v1.4**, all stamped 05-29-26 14:00. **Product-name abstraction (per the no-product-names standard):** every "Zoom" mention in the two Entity PRDs replaced with "the virtual meeting platform"; the external reference doc is now cited as "Virtual Meeting Platform Integration Architecture" rather than by its product-named repo path. **Field renames:** `zoomWebinarId`→`virtualMeetingWebinarId`, `zoomHostEmail`→`virtualMeetingHostEmail`, `zoomLastSyncedAt`→`virtualMeetingLastSyncedAt`, `zoomConfig`→`virtualMeetingConfig`, `zoomRegistrantId`→`virtualMeetingRegistrantId`; `attendanceCapturedAt` keeps its (already neutral) name. **Identifier-collision fix:** the five integration fields had been assigned `CR-EVENTS-MANAGE-DAT-034`–`037` (Event) and `-045` (Event Registration), which collided with existing Event Registration field IDs in the process document (event/contact links, registrationDate, registrationSource, contactType-append). They were reassigned to the next free block, **CR-EVENTS-MANAGE-DAT-049 through DAT-053**, and added to the process document's Section 8 (Data Collected) as a new "Virtual Meeting Platform Integration Data Items" subsection. **Semantic reconciliation:** the attendance workflow step (step 17) in both the process document and the Domain PRD now describes automated capture when a deployment enables the integration, with manual transcription as the fallback for non-integrated deployments and other platforms; the attendance-mechanism decision cell and the SDO Section 4.7 Attendance Capture Model were updated to match; CR-EVENTS-MANAGE-ISS-001 was rescoped from a blanket deferral to "platform-agnostic import for non-integrated platforms only" in all three documents that carry it (process doc, Event Entity PRD, CR Domain PRD). **Mechanical:** Change Log sections were added to the two Entity PRDs, the process document, and the SDO (none had one); a v1.4 row was appended to the Domain PRD's existing Section 8 Change Log. All four newly-built documents validate clean. The CR Domain PRD shows two pre-existing schema quirks (393 table cells with `shd` before `tcBorders`, and the `<w:zoom>` display-zoom setting missing its `percent` attribute) that are present identically in the git original (verified via `git show HEAD:`) and were left untouched per the minimal-changes principle. This entry supersedes the 05-05-26 "Zoom integration Entity PRD updates" note below: those fields are now product-neutral, upstream-consistent, and collision-free. The conditional design (manual baseline + optional bridge, gated on `virtualMeetingConfig`) is unchanged — EVT-DEC-011/012 still govern.

**Prior structural change (05-20-26):** CBM-EspoCRM-HowTo.docx rebuilt from May 11 backup after discovering pre-existing corruption in the repo version. The repo file had been committed as base64-encoded text (28,174 chars) rather than proper binary, and the encoded content decoded to a truncated ZIP missing the End-of-Central-Directory record — only ~58% of `word/document.xml` was recoverable (249,931 of 432,143 expected bytes). Investigation traced both issues to the initial bulk commit `4369195 "More YAML Changes"`; every other .docx in the repo is stored as proper binary and is unaffected. Doug's local Dropbox copy was also corrupted, along with several prior entries in Dropbox version history; the latest intact source was a May 11 ODT export at v1.3, Last Updated 05-11-26 19:21, used as the recovery baseline. The ODT delta against the May 10 .docx (v1.2) covered the addition of Section 2.9 "One-to-One Relationships — In Depth" with nine sub-sections. The v1.4 changes between May 12 and the corruption are unrecoverable. **Content added (v1.3 → v2.1):** three new top-level sections populated under the previously-empty "Security Configuration" H1: Section 12 EspoCRM Security Concepts (12.1 Overview, 12.2 Building Blocks (Users, Teams, Roles), 12.3 Action × Level Matrix, 12.4 Field Level Security, 12.5 Assignment Permission, 12.6 Multiple Assigned Users vs Collaborators, 12.7 Role Merging, 12.8 Roles via Team Attachment vs Direct Assignment, 12.9 Hiding Relationship Panels Using Field-Level Security, 12.10 The Sensitive Sub-Entity Pattern), Section 13 CBM Security Implementation (13.1 Architecture Summary, 13.2 Entity Setup, 13.3 Teams, 13.4 Standard User Role, 13.5 Sensitive Authorized Roles, 13.6 Sensitive Coordinator Roles, 13.7 Data Integrity Role, 13.8 User Assignment Patterns, 13.9 Workflow Rules, 13.10 Coordinator Stale-Access Dashlets, 13.11 Test Plan), and Section 14 User Auto-Assignment (14.1 Default Behavior, 14.2 The User Preference That Can Disable It, 14.3 Bulletproofing with a Before Save Formula, 14.4 Multiple Assigned Users, 14.5 Default Team Set, 14.6 Application to CBM) — Section 14 covers the three-layer behavior governing assignedUser auto-population (UI default, user preference toggle, Before Save formula) plus the assignedUsersIds variant for Multiple Assigned Users entities and Default Team Set as the team-equivalent mechanism. **Security architecture decided during the session:** Company is universally visible; three sensitive sub-entities (ClientProfile, SponsorProfile, PartnerProfile) one-to-one linked to Company with Multiple Assigned Users enabled, hidden by default via field-level Read=No on the corresponding link fields on Company in the Standard User role. Three Sensitive Authorized roles (Client/Sponsor/Partner) attached to three access teams (Mentors, Sponsor Coordinators, Partner Managers), each granting Read=Own and Edit=Own on its sensitive entity, with field-level Read=Yes on the corresponding link field unlocking the panel. Three Sensitive Coordinator roles per type assigned directly to users (no unified Master Coordinator role — Admin serves that purpose). Data Integrity role attached to Data Integrity Team granting Edit=All on Company (no sensitive access). Assignment Permission set to Team on Sensitive Authorized roles enabling peer-to-peer mentor handoff; No on Standard User and Data Integrity; All on Coordinators. Workflows are additive-only (Option A): on Company.assignedUser change, the workflow adds the new owner to the related sensitive entity's assignedUsers field but never removes anyone — Coordinator stale-access dashlets surface assignedUsers entries that no longer match the current Company owner for manual cleanup. The Sensitive Sub-Entity Pattern is documented in Section 12.10 as a reusable design pattern; CBM's implementation in Section 13 is its three-parallel-instance application. **Mechanical edits:** version 1.3 → 2.1, Last Updated 05-20-26 03:48, two new Change Log rows recording the rebuild + security content addition (v2.0) and the user auto-assignment addition (v2.1). File saved as proper binary .docx (56,692 bytes) fixing the latent base64 corruption. Document validated end-to-end: 1,388 paragraphs (1,075 baseline + 313 added across v2.0 and v2.1), all docx validations passed. **Pre-existing inconsistency noted but not fixed (per minimal-changes principle):** sub-section numbering within H1 sections is non-monotonic — Email Configuration starts at sub-section 5 and Integration Configuration starts at sub-section 9 rather than continuing the sequence. Flagged as a separate cleanup pass. **Open work:** (1) Replace `PRDs/CBM-EspoCRM-HowTo.docx` in the GitHub repo with the new binary v2.0; this commit also resolves the latent base64 corruption. (2) Deferred YAML question: whether the CRM Builder YAML schema can express security configuration (roles, teams, field-level permissions, Assignment Permission, workflow rules). Per `dbower44022/crmbuilder` CLAUDE.md, Category 6 (Roles, field-level permissions) is deferred to YAML schema v1.2 — confirmed the question deserves its own focused session. (3) Document the Sensitive Sub-Entity Pattern as a reusable artifact in CRM Builder if/when the YAML schema gains security support.

**Prior structural change (05-05-26):** MN PRD verification pass and four-session remediation workpacket complete. The pass compared the Mentoring Domain PRD v1.0 against current MN-* YAML files in `programs/MN/` and surfaced seven findings plus one bonus observation: HIGH severity findings #1 (Engagement entity missing requestedMentor field — added to MN-INTAKE v2.2 / MN-MATCH v2.2 but never propagated to YAML, Engagement Entity PRD, or Domain PRD) and #2 (Domain PRD v1.0 reconciled from MN-INTAKE v2.1 and MN-MATCH v2.1, but current versions are v2.4 and v2.2 respectively); MEDIUM severity findings #3 (meetingCadence field marked Required:Yes but not populated at intake — would block Engagement creation), #4 (MN-INTAKE v2.3+ cross-domain workflows REQ-010 through REQ-013 missing from MANUAL-CONFIG), and #7 (Domain PRD lacks consolidated Entity Relationships sub-section in Section 4); LOW severity findings #5 (DAT-NNN field IDs renumbered between MN-INTAKE v2.1 and v2.4, breaking traceability), #6 (applicantSince vs applicantSinceTimestamp name mismatch — MN-INTAKE used shorthand, canonical is the longer form per Account Entity PRD v1.8 and CR-Account.yaml); and the Bonus observation (Domain PRD v1.0 missing clientNotes field from Section 4 Client Organization table). Four-session remediation workpacket authored and committed to the repo as UPDATE-PROMPT files (b2c6c11, 77b8e23) then executed in sequence: Session 1 (commit 990a858) — Engagement Entity PRD v1.2 → v1.3 adding requestedMentor field with parallel structure to engagementAssignedMentor and relaxing meetingCadence to Required:No with workflow-validation deferral, MN-Engagement.yaml v1.0.0 → v1.0.1 declaring engagementRequestedMentor as manyToOne relationship and changing meetingCadence to required:false, MN-MANUAL-CONFIG.md gaining MN-MC-CR-006 capturing the meetingCadence workflow validation; Session 2 (commit 6a6a140) — MN-MANUAL-CONFIG.md gaining MN-MC-AA-017 (Intake Contact dedup by email per REQ-010), MN-MC-AA-018 (Account.clientStatus → Applicant transition per REQ-011), MN-MC-AA-019 (Contact.prospectStatus → Converted per REQ-012), MN-MC-AA-020 (intake-time howDidYouHearAboutCbm layered write per REQ-013), CR-MANUAL-CONFIG.md gaining CR-MC-AA-017 (sourceAttributionDetails override audit-trail) and a cross-reference on CR-MC-AA-003 to MN-MC-AA-018; Session 3 (commit 10eb312) — MN-INTAKE.docx v2.4 → v2.5 renaming applicantSince to applicantSinceTimestamp throughout (REQ-011 body, Section 8 DAT-027 row, change log entry); Session 4 (commit 60f009b) — CBM-Domain-PRD-Mentoring.docx v1.0 → v1.1 re-reconciled from current process doc and Entity PRD versions (MN-INTAKE v2.5, MN-MATCH v2.2, MN-ENGAGE v2.3, MN-INACTIVE v1.2, MN-CLOSE v1.1; Engagement Entity PRD v1.3, Session Entity PRD v1.1, Account Entity PRD v1.8, Contact Entity PRD v1.7) with new Section 4 opening Entity Relationships sub-section containing 12 rows (Account ↔ Contact native, Account → Engagement, six Engagement-Contact relationships, Engagement → Account Referring Partner, Engagement → Session native parent link, three Session relationships) plus three explanatory notes covering the native relationships and the Referring Partner ownership; Section 4 Client Organization table gaining clientNotes and applicantSinceTimestamp rows; Section 4 Engagement table gaining Requested Mentor row and Meeting Cadence Required:No update; new "Entity: Client Contact (CR-MARKETING fields touched by MN-INTAKE)" sub-section listing prospectStatus, howDidYouHearAboutCbm, and sourceAttributionDetails as read/touch references to CR-domain-owned fields; eight v1.1 reconciliation decisions DEC-001 through DEC-008 appended to Section 5; MN-INTAKE-ISS-002 added and immediately closed in Section 6 (resolved by CR-MARKETING integration); new Section 7 Change Log added with v1.0 and v1.1 entries. Finding #5 (field-ID drift) resolved via Option 1 (append-only DAT-NNN convention adopted, citing v2.1 IDs to match current YAMLs and Engagement Entity PRD v1.3, with future MN process doc revisions following the append-only rule going forward); the decision is captured in MN-RECON-V1.1-DEC-008 and the Change Log. All eight findings closed. The four UPDATE-PROMPT files are retained as source-of-record for the workpacket: `PRDs/entities/UPDATE-PROMPT-Engagement-Field-Corrections.md`, `PRDs/UPDATE-PROMPT-MN-CR-CrossDomain-Workflows.md`, `PRDs/MN/UPDATE-PROMPT-MN-INTAKE-v2.5.md`, `PRDs/MN/UPDATE-PROMPT-CBM-Domain-PRD-Mentoring-v1.1.md`.

**Methodology contribution from 05-05-26:** The consolidated Section 4 "Entity Relationships" sub-section pattern is a new structural convention for Domain PRDs. Domain PRD v1.0 listed relationships only as relationship-typed fields scattered across per-entity field tables, which made the relationship structure invisible at the Domain PRD level even though it was complete in the Entity PRDs. v1.1 surfaces relationships as a first-class structural element opening Section 4, with required columns Relationship | Related Entity | Link Type | YAML location | Domain(s); rows filtered to domain-relevant relationships including native (no YAML), custom-entity-owned (declared in MN-* YAML), and cross-domain-contributed (e.g., Referring Partner); plus explanatory notes covering native relationships and cross-domain contributions. This convention should be propagated to MR Domain PRD, CR Domain PRD v1.2, and FU Domain PRD v1.0 at the next opportunity (likely as part of a future revision rather than dedicated edits, to avoid version churn). The convention also informs future Domain PRD authoring per the Phase 7 Domain Reconciliation guide — `crmbuilder/PRDs/process/interviews/guide-domain-reconciliation.md` should be updated to specify Entity Relationships as a required Section 4 sub-section.

**Post-workpacket follow-on (05-05-26 — late afternoon and evening):** Three follow-on items resolved or queued after the four-session remediation workpacket landed. (1) Finding #6 closure path executed as a forward rename — `applicantSince` → `applicantSinceTimestamp` adopted as the canonical field name everywhere it appears, matching the `datetime` type via the `Timestamp` suffix and aligning with the longer name already in use across MN-INTAKE v2.5, MN-Engagement.yaml 1.0.1, MN/CR MANUAL-CONFIG entries, CR-Account.yaml, and Mentoring Domain PRD v1.1. Two follow-up session prompts authored and committed to the repo: `aeab640` PRDs/entities/UPDATE-PROMPT-Account-Entity-PRD-v1.9-applicantSinceTimestamp-rename.md (one-cell rename in Section 3.3 plus header bump and ACT-DEC-015 entry), and `480b1a1` PRDs/CR/EVENTS/UPDATE-PROMPT-CR-EVENTS-CONVERT-v1.1-applicantSinceTimestamp-rename.md (21 occurrences across seven sections plus Depends On line update for the Account Entity PRD reference v1.3 → v1.9; this is a substantively distinct rename and was deferred from the workpacket per the technical-debt note in Domain PRD v1.1's Change Log). The Account Entity PRD prompt was executed as commit `a4fcac6` producing v1.9; the CR-EVENTS-CONVERT prompt was executed as commit `ec066af` producing v1.1 (22 occurrences of `applicantSince` renamed across Sections 1, 4, 5, 6, 7, 8, 10, and 11; Depends On line updated for the Account Entity PRD reference v1.3 → v1.9 with the broader Depends On freshness review for the other entries deferred to a future dedicated session; a new "V1.1 applicantSinceTimestamp Rename" sub-section appended to Section 11 Interview Transcript as a Q/A/Decision triple, mirroring MN-INTAKE v2.5's convention of recording version provenance via topic sub-headings within Section 11 rather than via a separate Change Log section, and elevating that convention from "MN-INTAKE precedent" to "established process-doc rename convention" for future renames). With both renames now executed, the deliverable PRD set is fully consistent on the field name; only historical UPDATE-PROMPT and SESSION-PROMPT files retain the short form, which is acceptable and intentional. (2) Methodology-contribution propagation queued: a fifth UPDATE-PROMPT was authored at `7abb505` PRDs/UPDATE-PROMPT-Domain-PRD-Entity-Relationships-Propagation.md to propagate the new Section 4 Entity Relationships sub-section pattern to MR Domain PRD, CR Domain PRD v1.2, and FU Domain PRD v1.0. Executed as three sequenced commits during this same evening session: `105f259` (MR Domain PRD v1.1 → v1.2 with a 4-row Entity Relationships table covering Account ↔ Contact, Dues → Contact, and the cross-domain consumption rows for Engagement → Contact (Assigned Mentor) and Session ↔ Contact (Mentor Attendees) read-only by MR-MANAGE/MR-DEPART; plus a new Section 7 Change Log added to MR which previously lacked one), `034ae23` (CR Domain PRD v1.2 → v1.3 with a 15-row consolidated Entity Relationships table spanning all four CR sub-domains — CR-PARTNER, CR-MARKETING, CR-EVENTS, CR-REACTIVATE — in a single table with sub-domain codes in the Domain(s) column to disambiguate; the new sub-section is inserted as an unnumbered Heading2 before the existing 4.1 Contact Entity to avoid cascading renumbering), `ccf30f8` (FU Domain PRD v1.0 → v1.1 with a 6-row Entity Relationships table including five FU-owned relationships from FU-Account.yaml and FU-Contribution.yaml plus one cross-domain MN-consumption row for the Engagement → Account (Client Organization) relationship that anchors FU-REPORT territory attribution; FU's saved-view union for the Active Donors and Funders Sweep List is explicitly excluded from the table per the methodology guide v1.6 scope rules and documented in an explanatory note). All three Domain PRDs continue at their prior Status (MR Draft, CR and FU Approved — Phase 8 Stakeholder Review complete (05-01-26)); per the Approved-document handling pattern, structural sub-section additions of this nature do not require re-review. Each commit is independently reviewable and revert-able. The Phase 7 Domain Reconciliation guide update (specifying Entity Relationships as a required Section 4 sub-section in `crmbuilder/PRDs/process/interviews/guide-domain-reconciliation.md`) was executed as `dbower44022/crmbuilder` commit `bd89a1f`, bumping the guide from v1.5 to v1.6 with: a new ~80-line Entity Relationships sub-section specification at the start of `### Section 4: Data Reference` defining required columns (Relationship, Related Entity, Link Type, YAML Location, Domain(s)), required content scope (YAML-declared relationships, native platform relationships, native parent links, cross-domain-contributed relationships) with explicit filter-out rules (other-domain relationships and inverse-side rows), required explanatory notes covering native relationships and cross-domain contributions, source-compilation instructions with pre-/post-Phase-9 YAML location conventions, and a citation of the MN Domain PRD v1.1 (CBM commit `60f009b`, 05-05-26) as the precedent; an updated "What the Domain PRD Must Contain" Section 4 row description; an updated Step 3 Review walkthrough Data Reference summary line; and a v1.6 Changelog entry. The companion UPDATE-PROMPT file is retained in the crmbuilder repo at `PRDs/process/interviews/UPDATE-PROMPT-Domain-Reconciliation-Guide-Entity-Relationships.md` as source-of-record. With this guide update committed and the three Domain PRD propagations (`105f259`, `034ae23`, `ccf30f8`) executed, the methodology-guide v1.6 propagation forms a fully-closed loop: the MN precedent is canonical, the methodology guide encodes the convention as a forward requirement, and all three pre-v1.6 Domain PRDs (MR, CR, FU) are now retroactively brought into alignment. (3) Two cross-conversation Session 4 reconciliations of the Mentoring Domain PRD v1.1 produced substantively-equivalent outputs from independent generators; the upstream commit `60f009b` is canonical, with the parallel-session output retained as an internal cross-check artifact and not committed. Across all five committed prompts plus the three executed renames (Session 4 Domain PRD v1.1, Account Entity PRD v1.9, and CR-EVENTS-CONVERT v1.1), Findings #1 through #7 plus the Bonus observation are now closed at the artifact level rather than the documentary level — Finding #6 in particular has moved from "acknowledged in the Change Log as technical debt" to "renamed downstream" for both the Account Entity PRD (v1.9) and CR-EVENTS-CONVERT (v1.1).

**PRD review closure pass (05-16-26 — late evening):** MN-INTAKE v2.6 → v2.7 closing Findings #1 through #6 from the same structural and content review that produced v2.6 earlier the same evening. **(1) Finding #1 — Change Log section.** New Section 12 Change Log added with a nine-row table (Version | Date | Changes) covering v1.0 through v2.7. Format mirrors the Domain Product Requirements Document precedent (MR Domain PRD v1.2 Section 7, MN Domain PRD v1.1 Section 7, etc.). Section 10 (Updates to Prior Documents) retained as the narrative companion source — the table is at-a-glance, the narrative is the full story. **(2) Finding #2 — `draw.io` product name.** The Section 4 workflow diagram callout had read "Workflow diagram to be inserted. Export from draw.io as PNG." The second sentence (authoring-tool instruction) was dropped entirely, leaving only "Workflow diagram to be inserted." This removed both the product name and the implementation-specific PNG guidance, per the user preference that product names belong only in implementation documentation. **(3) Finding #3 — view name mismatch.** Section 4 Step 4 said "New Client Submissions filter"; REQ-004 said "Submitted Applications view" (and the latter wording had already propagated to the Mentoring Domain PRD via REQ-004 inheritance). Section 4 Step 4 was reconciled to "Submitted Applications view" — the outlier aligned with the already-canonical term, avoiding any Domain PRD carry-forward churn. **(4) Finding #4 — email template name in workflow.** Section 4 Step 3 had referenced "the New Client Submission email template." Investigation showed the MR process documents (MR-RECRUIT, MR-APPLY, MR-MANAGE, MR-DEPART, MR-ONBOARD) all describe email notifications generically without naming specific templates; the templates themselves are declared at the YAML level in MR-Contact.yaml with HTML bodies in `programs/MR/templates/`. MN-INTAKE was the outlier. Generalized to "The Client Administrator receives an automatic email notification that a new submission has been received." Matches the MR-domain methodology of describing email *behavior* at the process-document level while the template *artifact* lives in YAML. **(5) Finding #5 — Open Issues table header.** Section 9 column header changed from "Requirement" (a copy-paste artifact from the Section 6 System Requirements table) to "Open Issue", matching the MN-ENGAGE and MN-CLOSE convention. **(6) Finding #6 — Section 11 V2.0 Restructuring wording.** The subsection body had read "This version restructures the document from the original six-section format to the v2.4 eleven-section format." Two problems: "This version" ambiguously read as the current document version when scanning, and "v2.4" was stale. Rewrote to past-tense with explicit subject and drift-proofed version reference: "V2.0 restructured the document from the original six-section format to the current eleven-section format. ..." **Same-revision date correction.** The v2.6 work earlier this evening was stamped 05-05-26 (a same-conversation timestamp error — the date was carried over from the v2.5 stamp rather than checking today's actual date). The v2.7 revision fixes the v2.6 Last Updated stamp in MN-INTAKE.docx Section 10 narrative header retroactively to 05-16-26, the actual date of both v2.6 and v2.7 work. The v2.6 row in the new Change Log table is therefore stamped 05-16-26 20:05, and the v2.7 row is stamped 05-16-26 04:02. No requirement or behavior changes across either revision. All ten findings from the MN-INTAKE PRD review are now closed (#7 informational; #8 informational; #9 and #10 closed by v2.6; #1 through #6 closed by v2.7). Committed as `8a6b076`. **Methodology-guide propagation closure (same evening).** The process-document specification update flagged as pending in the v2.6 entry has now been executed at `dbower44022/crmbuilder` commit `90735c4`, which bumps `PRDs/process/interviews/interview-process-definition.md` from v2.6 to v2.7. The guide now specifies two required Section 8 opening sub-sections for every future process document: Entity Relationships Established (columns Relationship | Related Entities | Link Type | PRD Reference | Established At) and Fields Touched From Other Process Documents (columns Field Name | Owning Entity | Canonical Reference | Populated By). A new Section 8.0 "Required Opening Sub-Sections in the Produced Document" was authored, Section 7.4 was extended with a reference-vs-write distinction, Section 8.5 was extended with an output-consolidation note, and the "What the Process Document Must Contain" table, Completeness Check, and Summary template were updated to reflect the new structure. Companion UPDATE-PROMPT retained at `dbower44022/crmbuilder` commit `d686c2f` (`PRDs/process/interviews/UPDATE-PROMPT-Process-Definition-Guide-Entity-Relationships.md`). Retroactive propagation to existing process documents in MN, MR, CR, and FU is intentionally out of scope — adoption is opportunistic at each document's next revision opportunity, mirroring the dispersed-adoption pattern used for the Domain Product Requirements Document level convention. With this guide update, today's methodology contribution forms a fully-closed loop at both the Domain PRD and process-document levels: MN-INTAKE v2.6 is the canonical process-document precedent, methodology-guide v2.7 encodes the convention as a forward requirement, and the pattern is now structurally parallel to methodology-guide v1.6 (Domain Reconciliation Guide, Section 4 Entity Relationships sub-section).

**PRD review follow-on (05-16-26 — evening review):** MN-INTAKE v2.5 → v2.6 from a structural and content review of the Client Intake process document. Two findings (#9, #10) addressed in a single revision. Section 8 (Data Collected) now opens with two new sub-sections before the existing per-entity tables. The first, **Entity Relationships Established**, lists the five structural relationships created by Client Intake (Client Contact ↔ Client Organization, Client Organization → Engagement, Primary Engagement Contact → Engagement, Engagement Contacts → Engagement, Requested Mentor → Engagement) with columns Relationship | Related Entities | Link Type | PRD Reference | Established At. The second, **Fields Touched From Other Process Documents**, lists the four externally-owned fields written to by this process: prospectStatus, howDidYouHearAboutCbm, and sourceAttributionDetails (CR-MARKETING Sub-Domain Overview v1.0 / Contact Entity PRD), and Engagement Contacts (MN-MATCH-DAT-021). MN-INTAKE-REQ-007 and the Engagement record description in Workflow Step 2 updated with cross-reference parentheticals pointing to MN-MATCH-DAT-021 as the canonical definition of the Engagement Contacts field. v2.6 narrative entry added to Section 10 (Updates to Prior Documents) per the existing scattered-version-history convention; addition of a formal Change Log section deferred to a separate revision. No requirement or behavior changes. This is the first instance of the Section 4 Entity Relationships methodology pattern (canonicalized in methodology-guide v1.6 at the Domain PRD level on 05-16-26) appearing at the process-document level; the equivalent process-document specification update to `crmbuilder/PRDs/process/interviews/interview-process-definition.md` is pending and is the natural propagation target for this convention. Six findings remain open from the same review and are deferred to subsequent revisions (#1 Change Log section, #2 draw.io product name in Section 4, #3 New Client Submissions filter / Submitted Applications view name mismatch, #4 New Client Submission email template named in workflow but not in REQ-003 or DAT row, #5 Section 9 Open Issues table column header mislabeled "Requirement", #6 Section 11 V2.0 Restructuring stale "v2.4 eleven-section format" wording). Two informational notes (#7 CLAUDE.md document-inventory MN-INTAKE row was stale at v2.4 — fixed in this same commit; #8 DAT-NNN field-ID drift remains as acknowledged technical debt per Finding #5 of the workpacket). Committed as `2c82d5b`. **Date correction:** the v2.6 entry was originally stamped 05-05-26 in error; the actual date of this work was 05-16-26. Stamp corrected retroactively in the v2.7 revision.

**Zoom integration Entity PRD updates (05-05-26 — late evening):** Two queued UPDATE-PROMPTs from the entities folder executed in sequence after the methodology-guide v1.6 propagation closed out. Both updates implement the field surface for Zoom Webinar integration as defined by the Zoom Integration Architecture PRD at `PRDs/product/zoom-integration-architecture.md` v1.0 in the `dbower44022/crmbuilder` repo (commit `57caeb7`). (1) `3c27a9b` — Event Entity PRD v1.0 → v1.1 adding four system-populated fields (zoomWebinarId, zoomHostEmail, attendanceCapturedAt, zoomLastSyncedAt) in a new Section 3.5 Zoom Integration sub-section, updating the existing virtualMeetingUrl and registrationUrl descriptions in Section 3.2 to specify the Zoom integration as the populating system for Virtual/Hybrid Events, extending the Section 5.1 Format-Driven Field Visibility bullets to add the four new Zoom fields to the Virtual and Hybrid cases, adding a new Section 5.4 Zoom Field Status-Driven Visibility documenting the layered status conditions (zoomWebinarId/zoomHostEmail/zoomLastSyncedAt require status ≠ Draft; attendanceCapturedAt requires status = Completed), inserting a new Zoom Integration layout panel in Section 6 between Documents Panel and Registrations Panel, appending Implementation Note 11 (Zoom integration field provisioning), and recording two new decisions EVT-DEC-011 (provisioning conditional on deployment YAML zoomConfig block) and EVT-DEC-012 (registrationUrl population source depends on format with Zoom integration enabled). (2) `517ee6d` — Event Registration Entity PRD v1.0 → v1.1 adding the single zoomRegistrantId field in a new Section 3.5 Zoom Integration sub-section, updating attendanceStatus description to document the Zoom-driven write-once writeback pathway plus the cancellation-triggers-Zoom-cancellation behavior, updating registrationSource description to document the Zoom-driven Walk-In auto-creation pathway from post-Webinar attendance reconciliation for matched Contacts, replacing the single overviewNote with two notes covering the now-three creation pathways and the three Zoom-driven field writes, appending a Zoom Integration layout panel after Cancellation Panel, adding Implementation Note 10 (Zoom integration field provisioning), and recording two new decisions EVREG-DEC-007 (write-once protection on attendanceStatus when bridge service writes terminal status) and EVREG-DEC-008 (Walk-In creation restricted to existing Contacts to avoid Contact-database pollution from one-off attendees). Both updates followed the established generator-driven Entity PRD workflow: edit `PRDs/entities/generate-{Entity}-Entity-PRD.js`, run with `node`, validate the regenerated docx, commit both the generator and the docx in a single commit. Toolchain observation for future sessions in environments other than Doug's local laptop: the Entity PRD generators hardcode `outputFile: "/home/claude/cbm/PRDs/entities/..."` (Doug's path layout); a symlink at that path → the actual repo clone makes the generators run without editing the hardcoded path, keeping the generator changes minimal and aligned with Doug's workflow. Both updates verified clean: docx validations passed, all new fields/descriptions/panels/notes/decisions present, no product names introduced, all Zoom Integration Architecture PRD section references resolve correctly. Of the six Entity PRDs originally flagged as deferred (Marketing Campaign, Campaign Group, Campaign Engagement, Segment, Event, Event Registration), two are now at v1.1 (Event, Event Registration) via these Zoom-driven updates and four remain at v1.0 without queued UPDATE-PROMPTs (Marketing Campaign, Campaign Group, Campaign Engagement, Segment). Whether those four need updates is open and would require an audit pass or explicit direction; no consistency gap is evident from the file listing alone.

 Five-file MN+CR-Account batch (`programs/CR/CR-Account.yaml`, `programs/MN/MN-Account.yaml`, `programs/MN/MN-Contact.yaml`, `programs/MN/MN-Engagement.yaml`, `programs/MN/MN-Session.yaml`) deployed end-to-end, exercising all the engine paths: native-entity field extension on Account (CR-Account, MN-Account), placeholder NO_WORK YAML (MN-Contact), large custom-entity creation with formulas and saved views (MN-Engagement: 19 fields, 6 relationships), and Event-type custom-entity creation with deferred-options multiEnum (MN-Session: 7 fields, 3 relationships). Final-state outcome: all 5 files validate clean and deploy successfully; all custom fields, layouts, and relationships present and verified server-side. Three CBM YAML fixes committed during the session. **CBM YAML changes today:** `11d5a5d` FU-Account.yaml v1.0.2 stripping the duplicate `type:link` field declaration on `assignedSponsorCoordinator` (link is correctly declared in the relationships: block; field-level metadata does not propagate to link records); `7b3414a` adding three test-minimal HTML body files at `programs/MR/templates/` (`mentor-application-confirmation.html`, `mentor-application-decline.html`, `mentor-duplicate-email-alert.html`) so MR-Contact's emailTemplates declaration validates and deploys — bodies are placeholder `TEST TEMPLATE` content with merge-field placeholders (`{{personalEmail}}`, `{{applicationDeclineReason}}`) intact; `ffee4ca` adding `optionsDeferred: true` to MN-Account.industrySubsector and MN-Session.topicsCovered (companion to crmbuilder commit `fb50b95` which added validator support for the new flag — the fields ship with empty options:[] by deliberate Phase 9 deferral, with operator post-deploy responsibility documented in MANUAL-CONFIG.md MN-MC-DD-002 and MN-MC-OL-001 respectively). **Engine work that landed in crmbuilder during the same session (nine fixes):** `3b3e9dc` layout writer skips c-prefix on custom-entity fields; `1115527` layout comparator compares name/width per item; `3daab49` auto-place required `name` field on detail layouts via `settings.autoPlaceName` default true (schema bumped to v1.2.3); `52abb94` STEP SUMMARY distinguishes NO_WORK from SKIPPED (new StepStatus.NO_WORK rendering as `NO WORK SPECIFIED`); `d98db71` validator resolves field references across sibling YAMLs (new ProgramContext value object carrying union of field names per entity across batch; new validate_program_with_context entry point preserving single-file fallback); `8345cd8` validator resolves EspoCRM native fields (new espo_impl/core/native_entity_types.py mapping native entities to base types, _native_field_names helper consuming existing audit_utils catalog); `fb50b95` validator supports `optionsDeferred:true` flag on enum/multiEnum (schema doc Section 6.3 + new Section 6.4.1 documenting deferred-options pattern with MANUAL-CONFIG.md companion-artifact rule); `e5f18fe` EntityManager.wait_for_metadata_ready polls GET /Metadata?key=entityDefs.{entity} after rebuild_cache until each named entity's metadata is materialized or 30s timeout (yellow-warns on timeout, doesn't fail; closes the async-rebuild race window); `e4ca6a6` removed ENTITY_NAME_MAP override entirely (three of five entries were wrong — Session->CSessions, Workshop->CWorkshops, WorkshopAttendance->CWorkshopAttendee — current EspoCRM applies a simple f"C{name}" rule with no pluralization or renaming; same fix applied symmetrically to INVERSE_ENTITY_NAME_MAP). **What this means for CBM going forward:** the engine is now demonstrably stable for the deployment patterns CBM uses; remaining Phase 9 deployments (MR-Contact + MR-Dues, 8 of 9 CR domain YAMLs, all 4 FU domain YAMLs) should now succeed without further engine fixes for these specific patterns. Two MN domain entities (Engagement, Session) are now live on the test instance with all custom fields, layouts, and relationships present. Manual-config items for those entities (6 saved views on Engagement; 1 duplicate-check rule on Account; deferred option lists on industrySubsector and topicsCovered) remain as documented operator post-deploy work per the MANUAL-CONFIG.md in each domain folder. **Late-session continuation (same 05-04-26 day):** deployment validation pass extended through MR and FU domains. MR (2 files: MR-Contact, MR-Dues) deployed clean in one batch with zero engine surprises — exercised emailTemplates (3 created end-to-end), workflows (4 surfaced as MANUAL_CONFIG), 5 saved views, 1 duplicateCheck, 5 formula-fields, 1 cross-entity relationship Dues→Contact. FU (4 files: FU-Account, FU-Contact, FU-Contribution, FU-FundraisingCampaign) deployed clean in a two-step manual sequence: FundraisingCampaign first, then the other three plus CR-Account and MR-Contact selected as already-deployed dependencies for cross-YAML field resolution. Final state: 11 of 19 YAML files validated end-to-end against a freshly-reset EspoCRM instance — MN (4/4 ✅), MR (2/2 ✅), FU (4/4 ✅), CR (1/9 — only CR-Account; remaining 8 deferred to future session). Live entities on the test instance: 5 custom entities created (CEngagement, CSession, CDues, CContribution, CFundraisingCampaign), 2 native entities extended with ~125 custom fields total, ~12 relationships, 3 emailTemplates, ~21 MANUAL_CONFIG items correctly surfaced for operator post-deploy work. **One additional CBM YAML fix during the FU work:** `a538b01` adding `optionsDeferred: true` to `FU-Account.geographicServiceArea` and `FU-FundraisingCampaign.geographicServiceArea` (same Northeast Ohio zip code master list deferral as `ffee4ca`'s MN flags; FU-MC-OL-001 documents the operator post-deploy responsibility). **Three additional crmbuilder engine fixes during the late-session continuation (Option 3 cleanup of engine-bug backlog):** `1464559` Configure log shows absolute path of YAML being processed (closes the diagnostic gap that cost ~10 minutes of investigation earlier in the session); `1d9bd0e` `phase_verify` polls network-dependent verification checks instead of probing once (60s per-check timeout, 1/1/2/2/3/3/5s+ backoff; benefits both Recovery & Reset and fresh deploy paths); `aeba0e6` `phase_post_install` reads cert expiry from disk rather than going through nginx port 443 (eliminates the misleading 'Could not read certificate from <stdin>' false-error in reset logs). **Three engine-improvement findings filed for next session, none blocking:** validator should consult server state for cross-batch field references (today's workaround was including dependency YAMLs in the batch); deploys should topological-sort by relationship dependencies rather than alphabetically (today's workaround was a manual two-step deploy when FU-Contribution targeted not-yet-deployed FU-FundraisingCampaign); FU-FundraisingCampaign re-declares native `description` field with conflicting type — engine correctly skips with TYPE CONFLICT, YAML should drop the redeclaration. Engine commit total today: 12 fixes. CBM YAML fix total today: 4. CR domain (8 of 9 files) is the remaining deployment-validation work for a fresh session.

**Process status: All MR domain work complete. MR Phase 9 follow-up work fully closed out (04-20-26). CR Domain Reconciliation (Phase 5) complete (04-16-26). CR Domain PRD v1.4 at PRDs/CR/CBM-Domain-PRD-ClientRecruiting.docx — Phase 8 Stakeholder Review complete (05-01-26) at v1.2; structural sub-section addition v1.3 (commit `034ae23`, 05-05-26) per methodology-guide v1.6 propagation does not require re-review; v1.4 CR-EVENTS virtual meeting platform integration reconciliation (05-29-26) propagates a previously-ratified conditional integration and likewise does not require re-review per the Approved-document handling pattern. FU domain Phase 4b process definition complete: FU Domain Overview v1.0 committed 04-22-26; FU-PROSPECT process document v1.0 committed 04-22-26; FU-RECORD process document v1.2 committed 04-30-26 (initial v1.0 04-29-26; v1.1 carry-forward executed 04-29-26 applying the hybrid acknowledgment ownership model surfaced by FU-STEWARD; v1.2 carry-forward executed 04-30-26 adding Fundraising Campaign.geographicServiceArea field surfaced during FU-REPORT process definition); FU-STEWARD process document v1.0 committed 04-29-26; FU-REPORT process document v1.1 committed 05-01-26 (initial v1.0 04-30-26; v1.1 carry-forward executed 05-01-26 aligning Section 7.3 FU-REPORT-DAT-027 giftType enum with the FU-RECORD-authoritative seven-value list per FU-RECON-DEC-001). Phase 7 FU Domain Reconciliation complete (05-01-26) producing FU Domain PRD v1.0 at PRDs/FU/CBM-Domain-PRD-Fundraising.docx. FU-RECON carry-forward bundle complete (05-01-26): FU-RECON-ISS-001 closed by FU-REPORT v1.0 → v1.1 carry-forward; FU-RECON-ISS-002 closed by Entity Inventory v1.5 → v1.6 carry-forward. FU-RECON-ISS-003 (Northeast Ohio zip code master list) remains as a Phase 9 deferral. FU Domain PRD v1.1 — Phase 8 Stakeholder Review complete (05-01-26) at v1.0; v1.1 structural sub-section addition committed 05-05-26 (commit `ccf30f8`). CR Domain PRD v1.3 — Phase 8 Stakeholder Review complete (05-01-26) at v1.2; v1.3 structural sub-section addition committed 05-05-26 (commit `034ae23`). Phase 9 YAML Generation complete for the FU domain (05-01-26): four YAML files (FU-Contact.yaml, FU-Account.yaml, FU-Contribution.yaml, FU-FundraisingCampaign.yaml) plus EXCEPTIONS.md and MANUAL-CONFIG.md committed to programs/FU/. Four exceptions surfaced and resolved: FU-Y9-EXC-001 (geographicServiceArea master list deferred — empty options, recorded as MANUAL-CONFIG FU-MC-OL-001), FU-Y9-EXC-002 (auto-Active transition not expressible as v1.1 cross-entity workflow — recorded as MANUAL-CONFIG FU-MC-AA-001), FU-Y9-EXC-003 (donorContact / donorAccount mutual exclusivity not expressible — recorded as MANUAL-CONFIG FU-MC-AA-002), FU-Y9-EXC-004 (Active Donors and Funders Sweep List spans two entities — recorded as MANUAL-CONFIG FU-MC-UV-001). Manual Configuration List has six Role-Based Field Visibility entries (FU-MC-RV-001 through FU-MC-RV-006), three Advanced Automation entries (FU-MC-AA-001 through FU-MC-AA-003), one cross-entity union view (FU-MC-UV-001), and one deferred-master-list option list (FU-MC-OL-001). The implementation-feasibility concern raised during Phase 8 review proved well-founded — four genuine v1.1 schema gaps surfaced during FU YAML generation; all four are recoverable via post-deployment Manual Configuration. Schema v1.2 candidates identified: cross-entity setField action, mutually-exclusive-field constraint construct, cross-entity union saved-view construct. CR and MR Phase 9 YAML Generation remain pending. Bundled end-of-FU-Phase-4b carry-forward executed 04-30-26 propagating six new fields into Contact Entity PRD v1.6 → v1.7 (donorStatus, donorNotes, donorLifetimeGiving, lastContactDate; new Section 3.8 Donor-Specific Fields) and Account Entity PRD v1.7 → v1.8 (assignedSponsorCoordinator, lastContactDate added to Section 3.4 Donor/Sponsor-Specific Fields). EI-ISS-001 closed by FU-RECORD with field-level acknowledgment model (acknowledgmentSent, acknowledgmentDate on Contribution, shared across all contribution types) under hybrid ownership: FU-RECORD primary at Contribution creation, FU-STEWARD catch-up during the sweep. ACT-ISS-004 closed by FU-REPORT process definition session — geographicServiceArea on Account restructured from text to multiEnum (structured zip code list) with visibility expanded to both Partner and Donor/Sponsor account types; same field added to Fundraising Campaign entity (FU-RECORD-DAT-047). CON-ISS-003 closed by bundled carry-forward (donorStatus added). ACT-ISS-002 closed by bundled carry-forward (FU-domain Account fields integrated). All five completed carry-forward request files at PRDs/FU/carry-forward/ are retained as the source-of-record for their decisions. Current versions of upstream documents: Master PRD v2.5, Contact Entity PRD v1.7, Account Entity PRD v1.9, Engagement Entity PRD v1.3, Session Entity PRD v1.1, Dues Entity PRD v1.1, MN-INTAKE v2.5, MN Domain PRD v1.1, CR Domain Overview v1.4, CR-PARTNER-MANAGE v1.1, Entity Inventory v1.6, CR Domain PRD v1.4 (Phase 8 Approved at v1.2; v1.3 structural addition 05-05-26; v1.4 CR-EVENTS integration reconciliation 05-29-26), FU Domain Overview v1.0, FU-RECORD v1.2, FU-STEWARD v1.0, FU-REPORT v1.1, FU Domain PRD v1.1 (Phase 8 Approved at v1.0; v1.1 structural addition 05-05-26), CR-EVENTS-CONVERT v1.1 (commit `ec066af`). All seven CR Phase 2b Entity PRDs complete: Event v1.2 (virtual meeting platform integration reconciled 05-29-26; integration fields originally added 05-05-26 commit `3c27a9b`), Event Registration v1.2 (virtual meeting platform integration reconciled 05-29-26; integration field originally added 05-05-26 commit `517ee6d`), and at v1.0 Partnership Agreement, Segment, Campaign Group, Marketing Campaign, Campaign Engagement. Both FU-owned Phase 5 Entity PRDs complete at v1.0 (committed 04-30-26): Contribution Entity PRD v1.0 and Fundraising Campaign Entity PRD v1.0. All Phase 5 Entity PRDs across all four domains are now complete. Next: Phase 9 YAML Generation for CR (in a fresh session for clean context) or MR Phase 9 completion (which had partial earlier work).**
...
|---|---|---|
| Client Intake | `PRDs/MN/MN-INTAKE.docx` | v2.7 |
...
| Document | File | Version |
|---|---|---|
| Mentoring Domain PRD | `PRDs/MN/CBM-Domain-PRD-Mentoring.docx` | v1.1 |
